...
N = 80; % number of collocation point.

% initial value of state (need to be 1 x n vector)
X0 = [0, 10000, 300, 0, 150]; % x0 = 0 km, h0 = 10 km, v0 = 300 m/s, gamma0 = 0 deg, m0 = 150 kg

% load solution with smaller N for initial guess for x, u, and tf
load("prev_sol_N75.mat")
...
m_guess = interp1(taus(2:end), X_LG(:, 5), col_pts, 'linear', 'extrap');
...
X_LG_guess = [x_guess, h_guess, v_guess, gamma_guess, m_guess];
...
% plot initial guess
plotInitialGuess(col_pts, x_guess, h_guess, v_guess, gamma_guess, m_guess, U_LG_guess)
...
F = getFmatrix(X_LG, U_LG, N, n, taus); % N x n matrix
